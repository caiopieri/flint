...
**Phase 1a in progress.** Done: T0 (scaffold), T1 (vault), T2 (SyncProvider + iCloud conflicts), T3 (CodeMirror editor over the bridge), plus T8 (editor Live Preview) and T9 (vault file management). **Next: T4 — full-text search (SQLite FTS5 via GRDB).** Track concrete state in [`docs/TASKS.md`](./docs/TASKS.md) (checkboxes + PR refs); follow the module structure in `docs/ARCHITECTURE.md`.
